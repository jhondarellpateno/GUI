...
@SuppressWarnings("unchecked")
    ...
    private void jToggleButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jToggleButton1ActionPerformed
        ...
        int confirm = JOptionPane.showConfirmDialog(null, "Confirm changes?", "Update", JOptionPane.YES_NO_OPTION);

        if (confirm == JOptionPane.YES_OPTION) {
            String sql = "UPDATE tbl_user SET u_name = ?, u_email = ?, u_image = ? WHERE u_id = ?";
            conf.updateRecord(sql, name, email, finalPath, userId);

            UserSession.getInstance().setU_name(name);
            UserSession.getInstance().setU_email(email);
            UserSession.getInstance().setImagePath(finalPath);

            JOptionPane.showMessageDialog(null, "Updated Successfully!");

            String userType = UserSession.getInstance().getU_type();
